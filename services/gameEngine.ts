...
export const updateGame = (
    cars: Car[],
    track: Segment[],
    input: { left: boolean, right: boolean, up: boolean, down: boolean, left2: boolean, right2: boolean, up2: boolean, down2: boolean },
    dt: number,
    totalLaps: number,
    callbacks: {
    ...
    // ... (rest of collision logic remains same, it uses car properties)
    cars.forEach(car => {
    ...
    let speedRatio = player.speed / player.maxSpeed;

    // 2. Lateral (Tangent Drift + Centrifugal)
    let playerSegment = track[Math.floor(player.z / SEGMENT_LENGTH) % track.length];
    let curve = playerSegment.curve;
    ...
    // 3. Steering Input
    let steeringInput = input.left ? -1 : input.right ? 1 : 0;
    if (steeringInput !== 0) {
    ...
    cars.forEach(car => {
        ...
        // Shared physics/environmental variables for both AI and Manual
        let distToPlayer = player.z - ai.z;
        if (distToPlayer < -trackLength / 2) distToPlayer += trackLength;
        if (distToPlayer > trackLength / 2) distToPlayer -= trackLength;

        // Drafting boost (Applied to both AI and Manual for fairness)
        ...
        if (ai.isManualControl) {
            // === MANUAL CONTROL MODE (Second Player) ===
            if (input.up2) {
                ai.speed += ai.accel * aiAccelMultiplier;
            } else if (input.down2) {
                ai.speed -= PHYSICS.BRAKING;
            } else {
                ai.speed -= PHYSICS.DECEL_COAST;
            }

            // Offroad friction
            const aiOffroadDist = Math.abs(ai.offset) - 0.75;
            if (aiOffroadDist > 0) {
                const depth = Math.min(1.0, aiOffroadDist / 0.5);
                ai.speed -= (depth * depth * PHYSICS.DECEL_OFFROAD);
            }

            ai.speed = Math.max(0, Math.min(ai.speed, ai.maxSpeed * aiMaxSpeedMultiplier));

            // Tangent Drift + Centrifugal
            if (ai.speed > 0 && aiCurve !== 0) {
                const geometricForce = aiCurve * aiSpeedRatio * PHYSICS.DRIFT_FACTOR;
                const centrifugalForce = aiCurve * (aiSpeedRatio * aiSpeedRatio) * PHYSICS.CENTRIFUGAL_FORCE;
                const totalDrift = (geometricForce + centrifugalForce) * dt;
                ai.offset -= totalDrift;
            }

            // Steering Input (Manual)
            const aiSteeringInput = input.left2 ? -1 : input.right2 ? 1 : 0;
            if (aiSteeringInput !== 0) {
                let effectiveRatio = Math.max(0.1, aiSpeedRatio);
                if (aiSpeedRatio > 0.8) {
                    const dampFactor = (aiSpeedRatio - 0.8) / 0.2;
                    effectiveRatio *= (1.0 - (dampFactor * PHYSICS.SPEED_STEERING_DAMPING));
                }
                const steer = aiSteeringInput * PHYSICS.STEERING_SPEED * effectiveRatio * dt;
                ai.offset += steer;
            }
            ...
            if (ai.evasionState === 'normal') {
                ...
                else if (isPlayerBlocking && distToPlayer < 1200) ai.evasionState = 'evading';
            }
            ...
            if (ai.evasionState === 'blocked') {
                ...
                if (distToPlayer < safetyGap) {
                    ...
                    const tSpeed = Math.max(minBlockedSpeed, player.speed * 0.85);
                    ai.speed = Math.min(ai.speed, tSpeed);
                }
            } else if (ai.evasionState === 'evading') {
                ...
                if (ai.speed > targetEvasionSpeed + 10) ai.speed -= PHYSICS.BRAKING * 0.5;
                else if (ai.speed < targetEvasionSpeed - 10) ai.speed += ai.accel * aiAccelMultiplier * 0.8;
            } else if (ai.evasionState === 'overtaking') {
                ai.speed += ai.accel * aiAccelMultiplier * 1.5;
                if (ai.speed > (ai.maxSpeed * aiMaxSpeedMultiplier) * 1.1) ai.speed = (ai.maxSpeed * aiMaxSpeedMultiplier) * 1.1;
            } else {
                if (ai.speed < targetSpeed) ai.speed += ai.accel * aiAccelMultiplier;
                else ai.speed -= PHYSICS.BRAKING * 0.5;
            }
            ...
            for (let n = 1; n <= repairScanDistance; n++) {
                ...
                for (const sprite of segment.sprites) {
                    ...
                    if (sprite.source === 'REPAIR' && ai.damage > 5) {
                        if (!repairKitToSeek) repairKitToSeek = sprite;
                    }
                    if (n <= scanDistance) {
                        if (sprite.source !== 'REPAIR' && Math.abs(ai.offset - sprite.offset) < (spriteW + 0.5)) {
                            if (!obstacleToAvoid) { obstacleToAvoid = sprite; distToObstacle = n; }
                        }
                    }
                }
            ...
            if (obstacleToAvoid) {
                ...
                const rapidOscillation = Math.sin(time * 8) * 1.5;
                if (playerSide === aiSide) targetOffset = -playerSide * 1.8;
                else targetOffset = rapidOscillation;
                const positionZigZag = Math.sin(ai.z * 0.05) * 1.2;
                targetOffset = (targetOffset + positionZigZag) / 2;
            } else if (ai.evasionState === 'overtaking') {
                targetOffset = player.offset > 0 ? -1.3 : 1.3;
            } else if (ai.evasionState === 'blocked') {
                targetOffset = ai.offset;
            } else {
            ...
            if (!obstacleToAvoid && Math.abs(ai.offset) > 1.4) targetOffset = 0;
            if (!obstacleToAvoid) targetOffset = Math.max(-1.8, Math.min(1.8, targetOffset));

            const diff = targetOffset - ai.offset;
            let steerPower = PHYSICS.STEERING_SPEED * 0.5;
            if (ai.evasionState === 'evading') steerPower *= 2.5;
            else if (ai.evasionState === 'overtaking') steerPower *= 2.0;

            if (obstacleToAvoid && distToObstacle < 5) steerPower *= 1.8;
